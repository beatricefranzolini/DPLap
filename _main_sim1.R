...
# This code reproduces the results of Simulation Study 2.
...
################################################################################
#Simulation setting 1 (4 components + Normal)
################################################################################
set.seed(0)
K_0 = 4                                     # number of components             
mus = c(-3, 0, 1.5, 3)                      # components' centers
sig = 1                                     # components  scale
V   = rbeta(K_0, shape1 = 1, shape2 = 2)    
prs = cumprod(V)/sum(cumprod(V))            # components' weights
...
for(n in sample_sizes){
  ...
  set.seed(n)
  y = rnorm(n,
            mean = sample(mus, size = n, prob = prs, replace = TRUE),
            sd   = sig)
  hist(y, # histogram
       border="gray",
  ...
  Skew_LAP[[i_count]] = dp_lap_mixmodel_normal_normal(y, K = 20, skew = TRUE, n_samp = 2000, 
                                                      b = 3*log(length(y)), 
                                                      seed = 0) 
...
time_summary[-1] = lapply(time_summary[-1] , round, digits = 4)
print(time_summary)
